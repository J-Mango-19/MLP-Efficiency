...
def train(X_train, Y_train, X_test, Y_test, lr, steps, batch_size, status_interval):
    ...
    for step in range(steps):
        ...
        if step % status_interval == 0:
            ...
            print(f'Steps: {step} | Train Accuracy: {train_acc} | Test Accuracy : {test_acc}')
...
def main():
    ...
    lr, batch_size, steps, display_start, display_end, status_interval = get_input(sys.argv)
    ...
    end_alloc = time.time()
    print(f"alloc time: {end_alloc - start_alloc} seconds")
    ...
    print(f'Inference of entire training set took {end_matrix - start_matrix} seconds') # Inference time isolated for comparison
