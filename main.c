...
void forward_pass(Layers *layers, Matrix *X, Weights *weights) { 
    ...
    // layer 1
    multiply_matrices(weights->W1, X, layers->Z1);
    copy_matrix_values(layers->Z1, layers->A1); 
    append_bias_factor(layers->A1);
    relu(layers->A1);

    // layer 2
    multiply_matrices(weights->W2, layers->A1, layers->Z2);
    copy_matrix_values(layers->Z2, layers->A2);
    append_bias_factor(layers->A2);
    relu(layers->A2);

    // layer 3 (output)
    multiply_matrices(weights->W3, layers->A2, layers->Z3);
...
void inference_one_example(Matrix *X_test, Matrix *Y_test, Weights *weights, int index) {
    ...
    Layers *layers = init_layers(X_example, weights);
    Deltas deltas; 
    init_deltas(&deltas, layers, weights, X_example);
    Transpose transpose;
    init_transpose(&transpose, layers, 1, weights, X_example);
    forward_pass(layers, X_example, weights);
...
int main(int argc, char *argv[]) {
    ...
    Layers *layers = init_layers(&X_in, &weights);
    Deltas deltas; 
    init_deltas(&deltas, layers, &weights, &X_in);
    Transpose transpose;
    init_transpose(&transpose, layers, preferences->batch_size, &weights, &X_in);
    Layers *layers_eval = init_layers(&X_train, &weights);
    Layers *layers_test = init_layers(&X_test, &weights);
    ...
    for (int i = 0; i < preferences->num_iterations; i++) {
        forward_pass(layers, &X_in, &weights);
        backward_pass(&X_in, layers, &weights, &Y_in, &deltas, &transpose);  
        update_weights(&deltas, &weights, preferences->lr);
        if (i % preferences->status_interval == 0) {
            forward_pass(layers_eval, &X_train, &weights);
            forward_pass(layers_test, &X_test, &weights);
